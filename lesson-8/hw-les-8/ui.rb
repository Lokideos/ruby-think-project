class UI
  ...
  def main_msg
    puts 'Greetings! Welcome to the program, which will help you ' \
        'to maintain your railway stations!'
    puts 'In this program you can manage your trains, your stations ' \
        'and your routes.'
    puts 'Now you will be tranfered to the Main Menu.'
  end

  def run_msg
    puts "Please type in 'trains' for managing your trains, " \
        "'routes' for managing your routes, " \
        "'stations' to managing your stations " \
        "or 'cars' to add or delete cars."
  end
  ...
  def manage_trains_selected_msg
    ...
    puts 'Here you can add new trains, assign route to train, ' \
        'add cars to train, detach them from train.'
    puts 'Also here you can move your train on the route.'
    puts
    puts "Please type in 'add' to add the train, 'add route' " \
        'to assign route to the train;'
    puts "type in 'move' to move train on his route " \
        "or 'cars' to add or detach car from train"
  ...
  def manage_trains_add_route_msg(trains, routes)
    ...
    puts trains_numbers(trains)
    puts
    print 'Currently there are several routes: '
    puts print_names(routes)
    puts
    puts 'Please type in train number and route name you want to assign ' \
        'to this train in the respectful order:'
  end
  ...
  def manage_routes_msg
    ...
    puts 'Here you can add new routes and add station ' \
        'to this route or delete them.'
    puts
    puts "Please type in 'add' to add new route, " \
        "'add station' to add station to this route,"
  ...
  def train_move_on_route_direction_msg
    puts "Type in 'forward' if you want to move your train forward " \
        "or 'back' if you want to move your train backward."
  end
  ...
  def add_correct_car_action_msg
    puts "Type in 'add' to add the car to the train, 'detach' to detach it " \
        "or 'occupy' to take seat or add cargo to one of the cars."
  end

  def manage_trains_cars_add_msg(cars_free)
    puts 'Please select car to attach to the train.'
    print 'Currently there are several cars availible to attaching: '
    puts print_car_ids(cars_free)
    puts
  ...
  def manage_trains_cars_add_existance_car_error_msg(train)
    puts "Operation of attaching car to train number #{train.number} " \
        'has failed.'
  ...
  def manage_trains_cars_detach_success_msg(train, car)
    puts "Car #{car.car_id} was successfully detached " \
        "from train #{train.number}."
  end

  def manage_cars_choose_car_msg(cars)
    print 'Currently there are several cars: '
    puts print_car_ids(cars)
    puts
  end
  ...
  def manage_trains_occupy_success_passenger_msg(car)
    puts 'Passenger has successfully taken the seat ' \
        "on car number #{car.car_id}."
  ...
  def manage_routes_add_route_msg(routes, stations)
    ...
    puts print_names(stations)
    puts
    puts 'Please type in name of the first and last stations '\
    'for the new route in the respectful order:'
  end
  ...
  def manage_routes_add_station_msg(routes, stations)
    ...
    puts 'Please type in route name you want to add station to ' \
        'and station name you want to add to the route.'
  ...
  def manage_routes_delete_station_input_station_msg(route)
    print "Currently there are several stations on route #{route.name}: "
    puts route.stations_names
    puts
    puts 'Please type in station name you want to delete.'
  end

  def manage_routes_delete_station_success_msg(station, route)
    puts "Station #{station.name} has been succesfully removed" \
        "from route #{route.name}."
  end

  def manage_stations_msg
    puts 'Your are now in the managing stations program section.'
    puts 'Here you can add new stations or observe trains which' \
        'are currently arrived on this station.'
    puts "Please type in 'add' to add new station 'observe' to observe" \
        "trains on station or 'list' to view list of existing stations."
    puts "Also you can type in 'trains' to observe trains attached" \
  ...
  def manage_cars_add_car_success_msg(car)
    puts "#{car.class.to_s.capitalize} car" \
    "#{car.object_id} was succesfully added."
  end
